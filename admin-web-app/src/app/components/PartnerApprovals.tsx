...
export function PartnerApprovals() {
    ...
    const synchronize = async () => {
        ...
        try {
            ...
            console.error("Vetting Sync Error:", err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const queue = useMemo(() => partners.filter(p => p.status === 'pending' || p.status === 'approved'), [partners]);
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-32 gap-6 bg-gray-50/50 rounded-[64px] m-8 border border-gray-100 font-sans">
    ...
    if (renderError) return (
        <div className="p-20 text-center space-y-4 font-sans">
            ...
            <h2 className="text-2xl font-black uppercase text-red-900">Compliance View Failure</h2>
            <code className="block p-4 bg-gray-900 text-primary rounded-xl text-xs">{renderError}</code>
            <button onClick={() => window.location.reload()} className="px-8 py-3 bg-black text-white rounded-xl font-bold uppercase tracking-widest">Restart Intelligence</button>
        </div>
    );
    ...
    try {
        ...
        console.error("PartnerApprovals Render Error:", e);
        setRenderError(e.message || "Unknown error detected in vetting view.");
        return null;
    }
}
