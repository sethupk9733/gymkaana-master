import { Shield, Bell, Lock, Globe, Database, Mail, Save, User as UserIcon, Building, CreditCard, Sliders, Smartphone, Code, Server, Zap, Search, ExternalLink, RefreshCcw, HardDrive, Key, ChevronRight, ShieldAlert } from "lucide-react";
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="relative group">
                ...
                <div className="absolute right-6 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-300 pointer-events-none group-hover:text-black transition-colors rotate-90">
    ...
    return (
        <div className="p-8 bg-gray-50 border border-transparent rounded-[40px] flex items-center justify-between hover:bg-white hover:border-gray-100 hover:shadow-xl transition-all group">
            ...
            <div className={`w-16 h-10 rounded-full p-1.5 transition-all cursor-pointer shadow-inner ${active ? 'bg-black' : 'bg-gray-200'}`}>
                <div
                    className={`w-7 h-7 bg-white rounded-full shadow-lg transition-transform ${active ? 'translate-x-[22px]' : 'translate-x-0'}`}
                />
            </div>
        </div>
    );
